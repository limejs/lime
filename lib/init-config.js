...
module.exports = function() {
  ...
  try {
    ...
    const envConf = {}
    try {
      this._logger.info('*config*', '当前 NODE_ENV:', computedEnv)
      envConf = require(path.join(config.paths.config, `${envFilename}.js`))
    }
    catch(err) {
    }
    const result = Object.assign(config, commonConf, envConf)
    this._logger.ok('*config*', `站点配置 ${envFilename}.js 加载完成!`)
    return result
  }
  catch(err) {
    this._logger.error('*config*', '站点配置加载出错. 请检查站点根目录是否放置相应的 config 文件并正确导出配置对象', err.message, err.errno, err.code)
    throw err
  }
}
