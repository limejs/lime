...
'get|put|post|patch|delete|del'.split('|').forEach(method => {
  LimeRouter.prototype[method] = function (pathname, handlerPath) {
    ...
    try {
      ...
      logger.warn(`路由: 加载 controller[${controller}] 失败`, err.message)
      logger.warn(`路由: ${method} ${pathname} 缺少路由对应的控制器 ${controller}@${action}, 该路由将返回404`)
      return
    }
    if (typeof handlerController[action] != 'function') {
      // throw new Error('controller中缺少路由定义的action')
      logger.warn(`路由: ${method} ${pathname} 缺少路由对应的action ${controller}@${action}，该路由将返回404`)
      return
    }
