...
class MyRouter extends Router {
  static(rootpath) {
    ...
    return async function (ctx, next) {
      debug('静态资源托管的路由执行了', 'rootpath', path.resolve(paths.routerDir, rootpath))
      try {
        console.log('发送文件', path.join(path.resolve(paths.routerDir, rootpath), ctx.path))
        let destFilename = ctx.params[0] || ctx.path
        return  send(ctx, destFilename, { root: path.resolve(paths.routerDir, rootpath) })
      }
      catch (err) {
        debug(err)
        // ctx.throw(err)
...
'get|put|post|patch|delete|del'.split('|').forEach(method => {
  MyRouter.prototype[method] = function (pathname, handlerPath) {
    ...
    try {
      debug('controller path: ', path.join(controllerDir, controller))
      handlerController = require(path.join(controllerDir, controller))
    } catch (err) {
      debug(`路由: ${method} ${pathname} --- controller中缺少路由定义的controller(${controller}), 该路由将返回404`)
      return
    }
    if (typeof handlerController[action] != 'function') {
      // throw new Error('controller中缺少路由定义的action')
      debug(`路由: ${method} ${pathname} --- controller中缺少路由定义的action(${action})，该路由将返回404`)
      return
    }
