...
process.on('uncaughtException', (err) => {
  console.log()
  printLog.error('全局捕获的异常', err.code, err.message)
  printLog.error(err)
  process.exit(0)
})
process.on('unhandledRejection', (err) => {
  console.log()
  printLog.error('全局捕获的Promise异常', err.code, err.message)
  printLog.error(err)
  process.exit(0)
})
...
const printLog = require('../example/plugins/plugin-global-logger').logger
